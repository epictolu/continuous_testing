...
class FileSystemWatcher
  ...
  def addDirectory(dir, expression="**/*")
    if FileTest.exists?(dir) && FileTest.readable?(dir) then
      @directories << FSWatcher::Directory.new(dir, expression, @ignored_directories)
    else
...
module FSWatcher
  ...
  class Directory
    ...
    def initialize(dir, expression, ignored_directories)
      @dir, @expression, @ignored_directories = dir, expression, ignored_directories
      @dir.chop! if @dir =~ %r{/$}
    end
...
#    end
		...
		def getFiles()
			...
			Dir[@dir + "/" + @expression].each do |path|
				track_file = true

				@ignored_directories.each do |ignored_directory|
					if (path.match("/" + ignored_directory + "/"))
						track_file = false
						break
					end
				end
